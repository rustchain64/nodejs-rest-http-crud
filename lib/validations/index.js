...
function validateCreateUpdateCodesRequest (request, response, next) {
  ...
  if (stock === null || isNaN(stock) || stock < 0) {
    console.log('stockmust be greater or equal to 0');
    response.status(422);
    return response.send('The stock must be greater or equal to 0!');
  }

  if (id && id !== request.params.id) {
    console.log('Id was invalidly set on request.');
    response.status(422);
...
function validateCreateUserUpdateRequest (request, response, next) {
  // logger.info('>>> Validate USER request body: ', Object.keys(request.body).length);
  ...
  // No need to check for no body, express will make body an empty object
  const { firstname, lastname, username, agentcode, persona, hash, password, id } = request.body;
  ...
  if (!hash) {
    response.status(422);
    return response.send('The hash is required!');
  }
  ...
  // if (lastname === null || isNaN(lastname) || lastname < 0) {
  //   response.status(422);
  //   return response.send('The stock must be greater or equal to 0!');
  // }
